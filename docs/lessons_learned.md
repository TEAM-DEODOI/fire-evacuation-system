...
## L-015: PyBullet `getContactPoints` 는 mass=0 body 끼리 비어 있음

**Symptom**: `PersonAgent` 캡슐 (mass=0, 정적/kinematic) 이 `building.urdf`
의 내부 partition (mass=0) 을 통과. `step_toward` 의 collision-veto 가 동작
안 함. 진단:

```
# 캡슐을 partition 안 (14.9, 5, 0.85) 에 직접 배치 후 검사:
p.performCollisionDetection(client)
p.getContactPoints(bodyA=capsule, bodyB=building, client)  -> [] (비어 있음)
p.getClosestPoints(bodyA=capsule, bodyB=building, distance=0.0, client)
  -> [(... distance=-0.250 ...)]   # overlap 정확히 감지됨
```

URDF 자체는 정상: `getCollisionShapeData` 가 9개 GEOM_BOX 를 보고, `rayTest`
가 partition west face (x=14.9) 에서 hit. **충돌 자체는 PyBullet 이 인식**.

**Root cause**: PyBullet 의 contact-point buffer 는 **dynamic-physics 솔버의
부산물**이다. 양쪽 body 가 모두 mass=0 (static/kinematic) 이면 솔버가 contact
를 계산·기록하지 않는다. `performCollisionDetection` 도 contact-pair 갱신만
할 뿐 본질을 바꾸지 못함. `getContactPoints` 는 그 비어 있는 buffer 를 그대로
반환.

`getClosestPoints` 는 별개의 broadphase + narrowphase 쿼리로, 호출 시점의
포즈에서 직접 거리 계산. 양 body 의 mass / 정적-여부 무관. distance ≤ 0 이면
overlap, distance > 0 이면 그만큼 떨어짐. 우리 use case (kinematic 캡슐 vs
정적 벽) 에 정확히 맞음.

**Fix**: `src/integration/person_agent.py::PersonAgent.step_toward` 에서
`getContactPoints` 를 **`getClosestPoints(distance=0.0)`** 로 교체. 빈
리스트면 통과, 결과 있으면 (음수 거리 = 침투) revert.

```python
overlaps = p.getClosestPoints(
    bodyA=self.body_id, bodyB=other, distance=0.0, physicsClientId=client,
)
if overlaps:
    # revert pose, return False
```

**Status**: Fixed in M2-mini (2026-05-14). 본 패턴은 PyBullet 의 kinematic
agent 와 정적 환경 (building, signs, props) 간 collision 검사에 그대로 적용
가능 — DroneSwarm 의 drone-vs-wall 충돌 검사에서도 동일하게 써야 함.

**Note**: 만약 future PersonAgent / drone 이 mass>0 dynamic body 가 되면
`getContactPoints` 가 다시 동작한다. 그러나 dynamic 으로 가는 순간 capsule
이 쓰러지고 미끄러지는 등 부작용이 생겨 우리 시뮬에 부적합하다 (D-025 의
"simplified movement model" 명시적 제외). kinematic + `getClosestPoints` 가
정답.

**Cross-ref**: `src/integration/person_agent.py`, `src/integration/scene.py`,
`docs/pybullet_integration_spec.md §6.5` (자주 발생하는 버그 표 — 본 항목
추가 권장).

---

## L-016 — Fluid mask 만으로는 외부 spawn 발생 (interior_mask 도입)

**Symptom**: D-026 cell-grid path-planning 도입 후 `spawn_agents()` 가
`data/processed/dataset.h5::mask` (FDS fluid mask) 의 z=3 fluid cell
중 exit-reachable component 에서 random sample 했으나, 시각화 (GIF) 에서
PersonAgent 들이 **건물 외부** (북측 open courtyard, 동측 외부 yard,
남쪽 외부 strip 등) 에 spawn 되는 현상 다수 발생.

**Root cause**: FDS fluid mask 는 "연기가 흐를 수 있는 공간 (= solid 가
아닌 곳)" 으로, 30×20 SLCF 영역 안의 외부 buffer / open courtyard / 두
building 사이 공간을 모두 fluid 로 표기한다. 또한 exits 가 building
boundary 에 있어서 외부 fluid 셀과 내부 fluid 셀이 **하나의 connected
component** 로 묶임 — `only_exit_reachable=True` 필터가 작동 안 함.

**Fix**: 새 `interior_mask` 도입 (`data/processed/interior_mask.npz`):
1. 46 FDS 시나리오를 순회하며 각 시나리오의 마지막 frame 에서 `risk >
   0.6` 셀을 OR-결합 (union). 핵심 직관: "**외부 공간은 연기가 빠져나가
   atmosphere 로 분산되므로 risk 가 누적되지 않는다**" — 사용자 통찰
   (2026-05-14).
2. Wall-bleed 노이즈 제거를 위해 fluid mask 와 AND.
3. `load_interior_mask()` 가 이를 spawn pool 로 사용.

**Threshold 선택**: 0.05 → 외부 셀 포함 (open door 누출 + interpolation
bleed). 0.30 → 단일 시나리오 기준 깨끗하지만 fire 위치 종속. **0.6 →
46-시나리오 union 으로 모든 내부 방 cover + 외부 누출 모두 제거** —
z=3 에 701 cells (fluid 1826 - outdoor 1125 = 701 spawnable).

**Status**: Fixed in 2026-05-14. 검증: 6 seed × 20 agents = 120 spawn
모두 interior 내부 (visualization GIF + spawn 단위 테스트).

**Side-effect on EXP-PATH-001 mini-sweep**: 외부 spawn 제거 후 H6 신호
가 훨씬 강해짐 — S1 evac 80% → 53% (interior 한정 시 fixed-sign 의
greedy navigation 이 maze 안에서 자주 막힘), S1 1500kW exposure 0s
→ mean 92s, S2/S3 100% evac · ≤1.4s exposure 유지. 사용자가 보던
"외부에서 즉시 evacuated 으로 카운트되던 agents" 가 사라지면서 baseline
이 정확해진 결과.

**Cross-ref**: `scripts/build_interior_mask.py` (union 생성),
`scripts/visualize_interior_mask.py` (PNG + JSON 출력),
`scripts/sweep_interior_threshold.py` (threshold 선택 진단),
`src/path_planning/building_graph.py::load_interior_mask`,
`src/integration/scenarios/_common.py::spawn_agents`,
`docs/decisions.md::D-028`.

---

## L-017 — Raw CO field 는 RiskMap 과 별도 클래스로 (StaticCOField)

**Symptom**: PersonAgent FED 활성화 (D-029) 작업 시 `StaticRiskMap` 만
가지고는 `accumulate_fed_co(co_ppm_along_path)` 에 넣을 CO ppm 을 얻을
방법이 없었다. RiskMap 은 T/V/CO 를 4-metric tenability 함수로 aggregate
한 ``danger ∈ [0, 1]`` 만 보유하므로 CO 의 raw ppm 정보가 손실됨.

**Root cause (design)**: RiskMap 의 abstraction 이 path-planning 의 weighted
A* 가 필요한 "위험도 스칼라" 에 최적화되어 있다 (interface_contracts.md
§2.1). FDS 의 3 채널 (T/V/CO) 을 aggregate 하는 게 핵심 목적. 하지만
ISO 13571 §7.3 FED 누적은 raw CO ppm 시계열이 필수.

**Fix**: 별도 클래스 `src/risk_map/co_field.py::StaticCOField` 도입.

* 인터페이스가 `StaticRiskMap` 과 대칭 (`query(xyz, t) → ppm`,
  `from_fds_dir / from_npy / save`).
* OOB / out-of-time 정책은 **다르게** 설정: 외부 / 시작 전 → `0 ppm`
  (atmosphere; safe default), 끝난 후 → 마지막 frame 값으로 clamp (연기는
  순간 사라지지 않음).
* 캐시 디렉토리도 분리: `results/cache/scenario_co_fields/<scenario>.npz`
  (시나리오당 ~1.3 MB).
* `_common.py::load_truth_co_field()` 가 `load_truth_risk_map` 과 같은
  fall-back 패턴으로 zero CO field 반환 (test smoke / FDS missing 시).

대안 (반려): `StaticRiskMap` 에 CO grid 도 함께 보유 — 간단하지만
"위험도 = aggregate" 라는 핵심 abstraction 을 망가뜨림. 향후 FNORiskMap /
DynamicRiskMap 으로 확장 시 CO 누락 가능성도 높음.

**Status**: Fixed in 2026-05-14 (B1 / M2-full + M3-full). 검증: co_field.py
7/7 self-test PASS. 새 캐시 3 시나리오 (1.3 MB each) 생성됨.

**Cross-ref**: `docs/decisions.md::D-029`, `src/risk_map/co_field.py`,
`src/risk_map/fed.py`, `src/integration/person_agent.py::accumulate_exposure`.
